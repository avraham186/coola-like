...
export const modes = ['STARTED', 'IN_PROCESS', 'DELAY', 'COMPLETED']
...
export const ModeChoosen = ({ handleChangeStatus }) => {
    return modes.map((mode, i) => {
        return <span onClick={handleChangeStatus} key={i} name='Status' value={mode} id={`task-mode${i + 1}`}>{mode}</span>

    })
}
export const PriorityChoosen = ({ handleChangeStatus }) => {
    return priorities.map((priority, i) => {
        return <span onClick={handleChangeStatus} key={i} name='priority' value={priority} id='priority-task'>{priority}</span>
    })
}
